...
| **P5. 로테이션 지원** | ✅ **DEK 로테이션은 운영 검증까지 완료 (2026-08-20)**, KEK 로테이션 트리거는 대기 | KEK 자체를 버전 관리(이전에는 KEK가 단일 값이라 로테이션 시 전체 장애 위험이 있었음 — 5절 참고) + DEK/KEK 재wrap 유틸 + 명령어 트리거 + 절차 문서화 | vault-crypto `v0.0.7` — `KekService` 버전 인식형으로 재작성, `KekProvider`/`VaultKekProvider`/`KekRotationSupport` 추가, `DekProvider`/`KekProvider`에 `retire(...)` 추가, `EnvelopeCryptoService.currentVersion()`/`versionOf()` 추가(재암호화 배치가 이미 최신 버전인 행을 건너뛰게 함). demoApp에는 `DekOpsRunner`/`DekReencryptionService`/`dek_ops_batch.sh`/`dek_ops_batch.bat`로 DEK 로테이션+재암호화 명령어 트리거가 실제 구현되어 운영 서버(board/user-pii 두 도메인)에서 검증 완료. **rotate와 reencrypt는 항상 별도 실행이어야 함**(같은 실행에서 합치면 무의미 — 코드가 `IllegalStateException`으로 거부). KEK 로테이션(`KekRotationSupport.issueNewKekVersion()`/`rewrapDomainDeks()`)은 아직 명령어 트리거가 없음(코드는 준비됨). 상세 절차: `KEY_ROTATION_RUNBOOK.md` |
...
- 구현은 vault-crypto 라이브러리 확장(P1) → demoApp 배선(P2) 순으로 진행했고, **기존 암호화 데이터 마이그레이션(P3)과 구 포맷 하위호환은 2026-08-19 결정으로 포기**했다(`users` 테이블 삭제 예정, `board`의 기존 암호화 데이터는 무시). P0(Vault에 KEK/DEK 실제 생성)와 P5의 DEK 로테이션+재암호화 기능(명령어 트리거 포함)까지 2026-08-20 기준 운영 서버에서 실제 검증 완료됐다. 남은 건 KEK 로테이션의 명령어 트리거(선택 사항, 코드는 이미 준비됨)뿐이다.
