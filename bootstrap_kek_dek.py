...
Generates locally (no network calls, no Vault access):
  - one KEK (32 random bytes) - the master key, used only to wrap/unwrap DEKs
  - one DEK per service domain (board, user-pii), each wrapped with the KEK
    using AES-256-GCM in the same IV(12B) + ciphertext + tag(16B) format used
    by vault-crypto's AesGcmCodec/KekService (com.xaan.vault.crypto.envelope).
...
def wrap(kek: bytes, plaintext: bytes) -> bytes:
    """AES-256-GCM wrap: IV(12B) + ciphertext + tag(16B), matching AesGcmCodec.java."""
    iv = os.urandom(12)
...
def main():
    ...
    print("=" * 78)
    print("1) KEK 저장 (마스터 키 - DEK를 wrap하는 용도로만 사용)")
    print("=" * 78)
    print(f"vault kv put -mount={MOUNT} {BASE_PATH}/kek \\\n  kek=\"{b64url(kek)}\"\n")

    print("=" * 78)
    print("2) 도메인별 DEK 생성 및 저장 (version=1, KEK로 wrap됨)")
    print("=" * 78)
    for domain in DOMAINS:
        dek = os.urandom(32)
        wrapped = wrap(kek, dek)
